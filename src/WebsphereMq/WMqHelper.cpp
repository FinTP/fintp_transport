...
WMqHelper::WMqHelper() : TransportHelper( MQ_MSG_ID_LENGTH ), m_QueueManagerCCSID( 0 ), m_QueueManagerEncoding( 0 ), m_ConversionNeeded( false ), m_MessageFormat( MQFMT_STRING ),
#if defined( AIX )
...
void WMqHelper::connect( const string& queueManagerName, const string& channelDefinition, const string& keyRepository, const string& sslCypherSpec, const string& sslPeerName, bool force )
{
	...
	if ( reconnect || !m_QueueManager.connectionStatus() )
	{
		...
		m_QueueManager.setConnectOptions( MQCNO_HANDLE_SHARE_NONE );
